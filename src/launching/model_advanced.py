...
Tips: you can use regex to select examples. For example: example_00[1-5]* example_[6,7,8]*. If you want to run all examples, please type '*'.")
    ...
    ngroup: Int = Field(default=0,description="Number of groups to run in parallel. If not set, all examples will be run in parallel.",ge=0)
...
class AdvancedModel(IOSet,comm_class.ConfigSet,comm_class.TrackingSet,RunSet,comm_class_exampleSource.ExampleSourceSet,BaseModel):
    ...  
...
def ReadSetting(logs:comm_class.myLog,opts:AdvancedModel,work_path,download_path):
    ...
    if comm_class_exampleSource.parse_example_source(upload_path=opts.IO_input_path,
                                                     example_source=opts.example_source,
                                                     download_path=work_path,
                                                     configs=opts,
