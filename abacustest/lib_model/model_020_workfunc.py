...
class WorkFuncModel(Model):
    ...
    def prepare_args(parser):
        ...
        parser.add_argument(
            ...
            default="auto",
            choices=["a", "b", "c", "auto"],
            help="The direction of the vacuum for work function calculation, default is auto. If set to auto, the direction will try to be determined from structure automatically.",
        )
        parser.add_argument(
            "--dipole-corr",
            action="store_true",
    ...
    def postprocess_args(parser):
        ...
        parser.add_argument(
            ...
            default="auto",
            choices=["a", "b", "c", "auto"],
            help="The direction of the vacuum for work function calculation, default is auto. If set to auto, the direction will try to be determined from input file and structure automatically.",
        )
        parser.add_argument(
            "--dft",
            default="abacus",
    ...
    def run_postprocess(self, params):
        ...
        for job in params.job:
            ...
            results, plot_path, pot_file, plot_data_file = post_workfunc_calc(
                job, jobtype=jobtype, vacuum_dir_specified=params.vacuum
            )
...
def prep_abacus_workfunc_calc(
    ...
    if dipole_corr:
        ...
        input_params['efield_pos_max'] = None # Override original electric field position
        input_params['efield_pos_dec'] = None # Override original range to decrease of electric field
        ...
        if vacuum_dir == "auto":
            print(f"Using the largest vacuum direction identified by the largest vacuum: %s" % direction)
            input_params["efield_dir"] = EFIELD_DIRECTION_MAP.index(direction)
        elif vacuum_dir in ["a", "b", "c"]:
            if direction != vacuum_dir:
                print(f"WARNING: The given vacuum direction is {vacuum_dir}, not the same with the one identified by the largest vacuum {direction}.")
            print(f"Using the provided vacuum direction {vacuum_dir}")
            input_params["efield_dir"] = EFIELD_DIRECTION_MAP.index(vacuum_dir) # Always use the provided vacuum direction
        else:
...
def post_workfunc_calc(
    ...
    if jobtype == "abacus":
        ...
        vacuum_dir_input = input_params.get("efield_dir", None)
        if vacuum_dir_input:
        ...
        stru_file = os.path.join(workfunc_job, input_params.get("stru_file", "STRU"))
        stru = AbacusSTRU.read(stru_file)
