...
class PostBand:
    ...
    @staticmethod
    def split_plotdata(band, line_points, x=None):
    ...
    def plot_band(self, band, kpt, filename, x=None, efermi = None):
        ...
        for i, (pos, band) in enumerate(zip(positions, band_values)):
            for iband in band:
                ax[0].plot(pos, iband, "-", color="red",linewidth=0.6)
                ax[1].plot(pos, iband, "-", color="red",linewidth=0.6)
